...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/auth/user/me`, {
          method: "GET",
          headers: {
  ...
  //   try {
  //     const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/auth/user/${id}`, {
  //       method: "DELETE"
  //     });
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="bg-gray-50 p-4">
          ...
          <button
            onClick={() => navigate("/")}
            className="w-full bg-orange-500 hover:bg-orange-300 text-white font-medium py-2 rounded-md transition-all duration-300"
          >
            Go to Home
          </button>
        </div>
